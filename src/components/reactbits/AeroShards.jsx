...
fn fs_main(@location(0) uv: vec2f, @builtin(position) pixel: vec4f) -> @location(0) vec4f {
  ...
  let alpha = select(1.0, clamp(length(foreground) * 2.5, 0.0, 1.0), post.background.a < 0.01);
  return vec4f(clamp(background + foreground, vec3f(0.0), vec3f(1.0)), alpha);
}
`;

const parseColor = (value, fallback) => {
  if (value === 'transparent' || value === 'none' || value === 'rgba(0,0,0,0)') {
    return [0, 0, 0, 0];
  }
